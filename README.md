...
learning models coupled with wavelet transform and bootstrapping**, by Saraiva et. al [(2021)](https://www.sciencedirect.com/science/article/abs/pii/S1568494621000041). The article main idea is to used Discrete Wavelet Transform (DWT) to decompose the time series used for training machine learning models. The results presented by the authors show an increase on MLP and SVM models' performance. The purpose of the models is to forecast the daily streamflow in Sobradinho Reservoir.

In this project, a similar idea is applied for forecasting the Affluent Natural Energy (or *Energia Afluente Natural*, in portuguese) at the Paraná River basin. The data is provided by the **Operador Nacional do Sistema** (ONS), a government agency responsible for the coordination and monitoring of electric power generation and transmission facilities connected to Brazil's national grid.
...
- [2. Setup](#2-setup)
  ...
  * [2.2. Clone the tutorial complete code](#22-clone-the-project-from-github)
- [3. Docker](#2-docker)
...
After installing Docker in your machine (see section [2](#2-docker)), open a terminal (for Windows' users, the **Windows Power Shell**)
